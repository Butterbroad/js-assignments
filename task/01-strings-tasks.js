...
function encodeToRot13(str) {
    const lowerSymbols = 'abcdefghijklmnopqrstuvwxyz';
    const upperSymbols = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
    let resultStr = '';

    const checkLowerCase = symbol => {
        return symbol.toUpperCase() === symbol;
    };

    const getEncodeSymbol = symbol => {
        let index = -1;
        const isLowerCase = !checkLowerCase(symbol);

        const getCurrentIndex = ind => {
            if (ind >= 0) {
                if (ind > 25) {
                    return ind - 26;
                };
        ...
        if (isLowerCase) {
            index = lowerSymbols.indexOf(symbol) + 13;

            return lowerSymbols[getCurrentIndex(index)];
        } else {
            index = upperSymbols.indexOf(symbol);

            if (index >= 0) {
                return upperSymbols[getCurrentIndex(index + 13)];
            } else {
                return symbol;
            }
        };
    ...
    const currentStr = str.trim().split(' ');

    for (let curStr of currentStr) {
        let str = '';
        for (let smbl of Array.from(curStr)) {
            str += getEncodeSymbol(smbl);
        }
        resultStr += str + ' ';
    };
